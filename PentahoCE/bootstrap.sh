...
if [ ! -f /vagrant/downloads/$PENTAHO_CE ]; then
    echo Downloading Pentaho CE
	mkdir -p /downloads/
	wget http://downloads.sourceforge.net/project/pentaho/Business%20Intelligence%20Server/5.1/$PENTAHO_CE -P /vagrant/downloads/
	mkdir -p /opt/pentaho
	unzip /vagrant/downloads/biserver-ce-5.1.0.0-752.zip -d /opt/pentaho
fi
